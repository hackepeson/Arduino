...
const byte pinData = 7; // Digital output pin to LED strip
const byte pinClock = 3;  // Digital output pin to LED strip
const int numLeds = 144*2; // Number of LEDs
...
void setup()
{
  ...
  LEDS.addLeds<WS2812,pinData,GRB>(leds,0, numLeds);
}
...
void loop()
{
  ...
  FastLED.setBrightness(beatsin8(4, 43, 150));
